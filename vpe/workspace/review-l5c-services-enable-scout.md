# L5C Review Gate: `journal services enable scout` + storage module
...
- `journal_not_initialized`: Journal config file is missing. Run `journal setup`, then retry.
- `unknown_service`: Unknown service name. Use `journal services enable scout`.
...
| `unknown_service` | argparse-level: `journal services enable <something-other-than-scout>` |
