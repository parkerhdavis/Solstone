...
# ---------------------------------------------------------------------------
# vLLM advisory checks
# ---------------------------------------------------------------------------


class TestVllmDockerAvailable:
    def test_ok_when_docker_present(self, doctor, monkeypatch):
        monkeypatch.setattr(doctor.shutil, "which", lambda name: "/usr/bin/docker")
        result = doctor.vllm_docker_available_check(args(doctor))
        assert result.status == "ok"
        assert "docker found" in result.detail

    def test_warn_when_docker_missing(self, doctor, monkeypatch):
...
class TestVllmNvidiaSmi:
    def test_warn_when_nvidia_smi_missing(self, doctor, monkeypatch):
        monkeypatch.setattr(doctor.shutil, "which", lambda name: None)
        result = doctor.vllm_nvidia_smi_check(args(doctor))
        assert result.status == "warn"
        assert "nvidia-smi not on PATH" in result.detail

    def test_ok_when_nvidia_smi_lists_gpus(self, doctor, monkeypatch):
    ...
    def test_fail_when_no_gpu_lines(self, doctor, monkeypatch):
        monkeypatch.setattr(doctor.shutil, "which", lambda name: "/usr/bin/nvidia-smi")
        monkeypatch.setattr(
            doctor,
            "run_probe",
...
class TestVllmServersReachable:
    ...
    def test_skip_when_no_journal(self, doctor, monkeypatch, tmp_path):
        monkeypatch.delenv("SOLSTONE_JOURNAL", raising=False)
        monkeypatch.setattr(doctor, "ROOT", tmp_path)
        result = doctor.vllm_servers_reachable_check(args(doctor))
        assert result.status == "skip"
        assert "no journal config found" in result.detail

    def test_skip_when_section_missing(self, doctor, monkeypatch, tmp_path):
    ...
    def test_skip_when_servers_empty(self, doctor, monkeypatch, tmp_path):
    ...
    def test_skip_when_unreadable(self, doctor, monkeypatch, tmp_path):
        journal = tmp_path / "journal"
    ...
    def test_ok_when_all_reachable(self, doctor, monkeypatch, tmp_path):
        journal = self._write_journal(
            tmp_path,
            {
                "providers": {
        ...
        monkeypatch.setattr(
            doctor.urllib.request, "urlopen", lambda *_a, **_kw: _Resp()
        )
        result = doctor.vllm_servers_reachable_check(args(doctor))
        assert result.status == "ok"
        assert "alpha" in result.detail and "beta" in result.detail

    def test_warn_when_one_unreachable(self, doctor, monkeypatch, tmp_path):
        journal = self._write_journal(
            tmp_path,
            {
                "providers": {
        ...
        def fake_urlopen(url, **_kw):
            if "8001" in url:
                raise doctor.urllib.error.URLError("connection refused")
            return _Resp()
    ...
    def test_warn_when_entry_missing_base_url(self, doctor, monkeypatch, tmp_path):
        journal = self._write_journal(
            tmp_path,
