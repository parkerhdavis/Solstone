...
def test_unpair_observer_revokes_record_and_does_not_double_remove(
    ...
    response = _post_unpair(env, {"device_label": "observer"})

    assert response.status_code == 200
    assert response.get_json() == {"unpaired": OBSERVER_FINGERPRINT}
...
def test_unpair_observer_already_revoked_removes_authorized_and_warns(
    ...
    response = _post_unpair(env, {"fingerprint": OBSERVER_FINGERPRINT})
...
def test_unpair_observer_save_failure_removes_authorized_and_logs_error(
    ...
    response = _post_unpair(env, {"device_label": "observer-save-fails"})
