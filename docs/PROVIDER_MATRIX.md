...
> **Migration complete.** The fork added an `aarch64-unknown-linux-gnu` CUDA pin so the `local` bundle runs GPU-offloaded on the DGX Spark (GB10), extended it to **image** (Qwen3.6-35B-A3B via `--mmproj`), and repointed production `generate`/`cogitate` from vLLM to `local`. vLLM has been removed (it was fork-only and superseded). **Audio is not served on the bundle** — llama-server serves no audio input — so audio stays on the Whisper STT pipeline. So on the Spark today: `local` serves text + image; Whisper serves audio; cloud providers cover tier fallback. Full history in the Obsidian `90-99 Agents/Transient/llama.cpp Local Bundle Migration Plan`.
...
| `local/qwen3.6-35b-a3b` (Q8_0 + mmproj) | ✓ | ◐ | ✓ (image, via mtmd `--mmproj`) | ✗ (the bundle serves no audio input; audio stays on Whisper) | Fork-only. The Spark's served local model + vision model for the describe pipeline. From `unsloth/Qwen3.6-35B-A3B-GGUF`; chosen over Nemotron Omni by the B3 head-to-head (see docs/FORK.md). |
...
| Screen frame categorize | Generate w/ vision (tier-routed) | journal-pluggable | needs a vision-capable provider (cloud, or `local/qwen3.6-35b-a3b` on the Spark) |
...
| Multimodal-augmented meeting summary (audio fusion) | audio-omni model — **no local option** (the bundle serves no audio input); cloud-only if ever wired | local-only | feature unavailable on the bundle; Whisper transcript + Generate covers the text path |
...
1. **Audio understanding has no local model.** Audio runs through the Whisper STT specialist path (transcript, no fusion with other modalities). The bundle serves no audio input (Qwen3.6 has no audio modality, and the prior Nemotron Omni's audio was rejected by llama-server — "audio input is not supported"), so there is no audio-in LLM on the bundle — the closest substitute is "Whisper transcribes, a Generate-tier LLM interprets the text," which loses the audio-direct capability. Revisit if a local audio-in model lands on the bundle.
...
3. **The cloud↔local fallback story is symmetric for text/cogitate and now vision, asymmetric for audio.** Text/cogitate/vision degrade gracefully — cloud or the `local` bundle cover each other (vision via `local/qwen3.6-35b-a3b` or a cloud VLM). Audio understanding has no local fallback: the substitute is "Whisper transcribes, Generate-tier LLM interprets the text."
