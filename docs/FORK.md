...
The fork is a dev/testing instance running on a DGX Spark (GB10, aarch64 Linux). As of the 0.5.4 merge, divergence clusters around two things upstream doesn't carry:
...
The fork's former largest divergence — a self-hosted aarch64-Linux **CUDA** llama-server build for the Spark — **converged in the 0.5.4 merge**: upstream now ships its own aarch64-Linux GPU build (Vulkan) and a Vulkan-only launch path, so the fork dropped the CUDA build and rides upstream's binary. See [Historical context](#historical-context). Everything else is small. For the capability/routing view of providers, see [PROVIDER_MATRIX.md](PROVIDER_MATRIX.md).
...
**Files:** `solstone/think/models.py` (`LOCAL_MODEL` → Qwen3.6-35B-A3B), `solstone/think/providers/local.py` (Qwen3.6-35B-A3B `LOCAL_MODEL_SPECS` entry + the benchmark interface), `solstone/apps/settings/local_bootstrap.py` (the served-model display label), and tests `tests/test_local.py`, `solstone/apps/settings/tests/test_local_bootstrap_routes.py`. (`local_install.py` and `supervisor.py` are now byte-identical to upstream — see [Historical context](#historical-context).)
...
- **Audio stays on Whisper.** The bundle serves no audio input through llama-server (`b9291`) — Qwen3.6 carries no audio modality — so audio runs through the Whisper STT pipeline.
...
> **Backend caveat — Vulkan, not CUDA (open follow-up, 0.5.4).** As of the 0.5.4 merge the Spark runs the bundled model on upstream's **Vulkan** aarch64 build, not the fork's former purpose-built CUDA binary (see [Historical context](#historical-context)). Qwen3.6-Q8 + mmproj-BF16 loads on the same `b9291` llama.cpp the Vulkan build is cut from, so the served-model choice is backend-independent — but Vulkan-vs-CUDA throughput on the GB10 is **unmeasured**. The fork's own benchmark harness exists to settle exactly this; a Vulkan-vs-CUDA pass on the Spark is the open follow-up that decides whether the CUDA build is ever worth re-diverging for.
